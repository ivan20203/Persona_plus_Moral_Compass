...
class BaseModel(ABC):
    ...
    def get_chat_messages(self, question, prompt_type="base"):
        ...
        if prompt_type == "base":
            ...
            messages = [
                {"role": "system", "content": "You are a believer in"},
...
class OpenModel(BaseModel):
    ...
    def setup(self):
        ...
        # Set up quantization parameters based on model registry configuration
...
class OllamaModel(BaseModel):
    """Model wrapper for serving responses via a running Ollama server."""

    def __init__(self, model_name: str, host: Optional[str] = None):
        self.model_name = model_name
        self.host = host or os.environ.get("OLLAMA_HOST", "http://127.0.0.1:11434")
        self.client = None
        self.model_id = model_name.split(":", 1)[1] if ":" in model_name else model_name
    ...
    def setup(self):
        ...
        try:
            ...
            raise ImportError(
                "The `ollama` python package is required for Ollama-backed models. "
    ...
    def generate_responses(self, messages, num_responses: int = 1):
        """Generate a single response by chatting with the Ollama server."""
        if self.client is None:
            raise RuntimeError("Ollama client not initialized. Call setup() first.")
        ...
        try:
            ...
            time.sleep(2)
            return f"ERROR: Failed to generate response: {exc}"
...
class ModelFactory:
    ...
    def create_model(model_name, api_key=None, base_url=None):
        ...
        if model_name.startswith("ollama:"):
            return OllamaModel(model_name=model_name, host=base_url)
        else:
