...
def check_alignment_ollama(judge, response: str, argument: str) -> bool:
    """
    Check if response aligns with the argument using an Ollama-hosted model.
    """
    if judge is None:
        return False
    ...
    verdict = outputs[0].strip().lower() if outputs else ""
    if "aligned" in verdict and not any(token in verdict for token in ("neutral", "against")):
        return True
    if any(token in verdict for token in ("against", "neutral")):
        return False
        ...
        for j in range(1, 6):
            ...
            if response_key in row and row[response_key].strip():
                if ollama_judge is not None:
                    is_aligned = check_alignment_ollama(ollama_judge, row[response_key], argument)
                else:
...
def main(args):
    ...
    if getattr(args, "ollama_model", None):
        if OllamaModel is None:
            logging.error("Ollama support is not available because OllamaModel could not be imported.")
            return
        judge_name = args.ollama_model
        if not judge_name.startswith("ollama:"):
            judge_name = f"ollama:{judge_name}"
        try:
